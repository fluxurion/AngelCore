...
void AngelScriptMgr::RegisterTrinityCoreAPI()
{
    ...
    for (char const* t : { "Unit", "Player", "Creature", "GameObject", "Spell", "AuraEffect",
                           ...
                           "CastSpellExtraArgs", "CastSpellTargetArg" })
...
void AngelScriptMgr::TriggerCustomHook_PostCharEnum(WorldSession* session)
{
    auto& hooks = _customHooks[static_cast<size_t>(CustomHookType::ON_POST_CHAR_ENUM)];
    for (auto& func : hooks)
    {
        if (!_context) break;
        int r = _context->Prepare(func);
